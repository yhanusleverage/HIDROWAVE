...
export function ProcedureJsonPreview({ title, value }: ProcedureJsonPreviewProps) {
  const { t } = useLanguage();
  ...
  const copy = async () => {
    try {
      ...
      setCopied(true);
      toast.success(p.jsonCopied);
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast.error(p.jsonCopyFail);
    }
  };
  ...
  return (
    <div className="bg-dark-card border border-dark-border rounded-xl p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionHeader title={title ?? p.jsonCompiled} subtitle={p.jsonPreviewSub} accent="warn" className="mb-0" />
        <button
          type="button"
          onClick={copy}
