...
/**
 * API para criar comandos Master (relés locais do ESP32 Master)
 * 
 * Esta API cria comandos na tabela relay_commands_master com status "pending".
 * O ESP32 Master busca esses comandos usando a função RPC get_and_lock_master_commands().
 * 
 * Campos importantes:
 * - relay_numbers: ARRAY de números de relés (0-15)
...
export async function POST(request: Request) {
  try {
    ...
    // ✅ Validar relay_numbers (0-15)
    for (const relayNum of relay_numbers) {
      if (typeof relayNum !== 'number' || relayNum < 0 || relayNum > 15) {
        return NextResponse.json(
          { error: `relay_number inválido: ${relayNum}. Deve ser 0-15` },
          { status: 400 }
        );
      }
    }
    ...
    const result = await createMasterCommandDirect({
      master_device_id,
      user_email,
      master_mac_address,
      relay_numbers,
