...
type SharedDeviceStatusChannel = {
  listeners: Set<DeviceStatusListener>;
  channel: RealtimeChannel;
};

const sharedDeviceStatusChannels = new Map<string, SharedDeviceStatusChannel>();

function deviceStatusChannelName(normalizedEmail: string): string {
  return `hidrowave-device-status-${normalizedEmail.replace(/[^a-z0-9]/g, '-')}`;
}

function notifyDeviceStatusListeners(
  entry: SharedDeviceStatusChannel,
  status: DeviceStatusChannelStatus,
  err?: Error
) {
  if (status === 'SUBSCRIBED') {
    console.log('[Realtime] device_status SUBSCRIBED —', entry.channel.topic);
  }
  ...
  entry.listeners.forEach((listener) => listener.onChannelStatus?.(status));
}

function getOrCreateSharedDeviceStatusChannel(normalizedEmail: string): SharedDeviceStatusChannel {
  const existing = sharedDeviceStatusChannels.get(normalizedEmail);
  if (existing) return existing;

  const listeners = new Set<DeviceStatusListener>();
  const channelName = deviceStatusChannelName(normalizedEmail);

  const handleRow = (row: DeviceStatusRow, type: 'update' | 'insert') => {
    if (!row?.device_id || !belongsToUser(row, normalizedEmail)) return;
    const event = { type, row } as DeviceStatusRealtimeEvent;
  ...
  const channel = supabase
    ...
    .subscribe((status, err) => {
      if (status === 'SUBSCRIBED') notifyDeviceStatusListeners({ listeners, channel }, 'SUBSCRIBED');
      if (status === 'CHANNEL_ERROR') {
        notifyDeviceStatusListeners({ listeners, channel }, 'CHANNEL_ERROR', err);
      }
      if (status === 'TIMED_OUT') {
        notifyDeviceStatusListeners({ listeners, channel }, 'TIMED_OUT');
      }
      if (status === 'CLOSED') {
        notifyDeviceStatusListeners({ listeners, channel }, 'CLOSED');
      }
    });

  const entry: SharedDeviceStatusChannel = { listeners, channel };
  sharedDeviceStatusChannels.set(normalizedEmail, entry);
  return entry;
}
...
export function subscribeDeviceStatusUpdates(
  ...
  const entry = getOrCreateSharedDeviceStatusChannel(normalizedEmail);
  const listener: DeviceStatusListener = { onEvent, onChannelStatus };
  ...
  return () => {
    ...
    if (entry.listeners.size === 0) {
      ...
      sharedDeviceStatusChannels.delete(normalizedEmail);
    }
  };
}
