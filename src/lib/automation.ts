...
export async function getUserDevices(userEmail: string): Promise<DeviceStatus[]> {
  ...
  // ✅ PASO 1: Buscar emails válidos da tabela users
  ...
  console.log(`📧 ${validEmails.size} emails válidos na tabela users:`, Array.from(validEmails));
  
  // Normalizar email do usuário
  const normalizedEmail = userEmail?.trim().toLowerCase() || '';
  const isUserEmailValid = normalizedEmail && validEmails.has(normalizedEmail);
  
  console.log(`🔍 Email do usuário é válido? ${isUserEmailValid}`);
  ...
  const userDevices = data.filter(device => {
    ...
    // ✅ 3. VÍNCULO EMAIL: Verificar se o email existe na tabela users
    if (!validEmails.has(deviceEmail)) {
      return false; // Email não existe na tabela users
    }
    ...
    // ✅ 5. Filtrar por email do usuário:
    // - Se usuário tem email válido, mostrar apenas dispositivos com esse email
    // - Se usuário NÃO tem email válido, mostrar TODOS os dispositivos com email válido
    if (isUserEmailValid) {
      // Usuário logado: mostrar apenas seus dispositivos
      return deviceEmail === normalizedEmail;
    } else {
      // Usuário não logado ou email inválido: mostrar todos com email válido
      return true;
    }
  });
...
export async function discoverAvailableDevices(userEmail: string): Promise<DeviceStatus[]> {
  ...
  // ✅ Buscar todos os emails válidos da tabela users
  const { data: usersData } = await supabase
    .from('users')
    .select('email')
    .eq('is_active', true);
  ...
  console.log(`📧 ${validEmails.size} emails válidos encontrados na tabela users`);
  ...
  const availableDevices = data.filter(device => {
    ...
    // ✅ NÃO mostrar dispositivos que pertencem a outros usuários válidos
    // Apenas mostrar dispositivos sem email (disponíveis para atribuir)
    if (validEmails.has(deviceEmail)) {
      return false; // Pertence a outro usuário válido, não mostrar
    }
...
export async function registerDeviceWithEmail(
  ...
  // ✅ VALIDAR: Verificar se email existe na tabela users
  ...
  if (userCheckError || !userCheck) {
    console.error('❌ Email não encontrado na tabela users:', normalizedEmail);
    return null;
  }

  if (!userCheck.is_active) {
    console.error('❌ Usuário está desativado:', normalizedEmail);
    return null;
  }
