...
function mapAuthErrorMessage(error: {
  ...
  if (msg.includes('failed to fetch') || msg.includes('networkerror')) {
    return 'Sem ligação ao Supabase — verifique NEXT_PUBLIC_SUPABASE_URL e ANON_KEY em .env.local (copie de ESP-HIDROWAVE-main/secrets.ini), apague .next e reinicie npm run dev.';
  }
  ...
  const signIn = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      if (msg.includes('failed to fetch')) {
        toast.error(
          'Sem ligação ao Supabase — revise .env.local e reinicie npm run dev.'
        );
      } else {
